...
#   arg2, if present = count, 0=infinite 
#   arg3, if present = interval in sec (float)
#
# todo:
#   args for stand-alone program
#   try socket, if listener-alive: then ping, othwise error
#   Bug? !! ping_delay_s causes errors when below 1ms ??
# 
...
print ( ' ---- tst_ping.py first do imports ..--- ' )
...
# testing to see if args is usable.
def f_tstargs ( *args ):

  pp ( 'args = -[', *args, ']-, of type:', type( *args ) )

  n_args = len ( *args )  

  pp ( 'tstarg nr args = ', n_args )

  #for n  in range ( 0, n_args ):
  #  pp ( 'arg[', n, '] =', args[n], ' of type:', type ( args[n] ) )

  n=0
  for arg in args:
    pp ( 'details:',  f"Value: {arg}, Type: {type(arg)}")
    pp ( 'arg[', n, '] =', arg, ' is of type:', type(arg) ) 
...
def ora_lsnr ( the_conn ):
  ...
  try:
    ...
    print("Cannot reach listener:", e)
    return -1
  
  return -2 # -- -- -- -- experiment with port-comm: check listener without logon -- -- --
...
def f_run_pings ( ora_conn, sleep_s=0.0, n_max_pings=20 ):
  ...
  # ping_delay_ms = 1000
  # delay has to be at least 2ms, otherwise causes errors???
  ...
  # pp ('-- ping delay was ', ping_delay_s, 's' ) 
...
# pp ( ' len argv = ', n_args )
#
# for n_loop in range (0,n_args):
#   pp ( ' args ', n_loop, '=',  sys.argv[n_loop] )
#
# -- here -- 
# f_tstargs ( [sys.argv, '-i3', '-c2' ] ) 


# pick SQL from commandline, if arg1: use it as SQL..
# if len(sys.argv) == 2:
#   arg1 = sys.argv[1]
# else:
#   print    ( ' ' )
